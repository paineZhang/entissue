...
type OrganizationQuery struct {
	...
	withUnits    *OrgUnitQuery
	withStaffs   *UserQuery
...
func (oq *OrganizationQuery) QueryCreateBy() *UserQuery {
...
func (oq *OrganizationQuery) QueryUpdateBy() *UserQuery {
	query := &UserQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (oq *OrganizationQuery) QueryUnits() *OrgUnitQuery {
	query := &OrgUnitQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) QueryStaffs() *UserQuery {
	query := &UserQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (oq *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withCreateBy: oq.withCreateBy.Clone(),
		withUpdateBy: oq.withUpdateBy.Clone(),
		withUnits:    oq.withUnits.Clone(),
...
func (oq *OrganizationQuery) WithCreateBy(opts ...func(*UserQuery)) *OrganizationQuery {
	query := &UserQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withCreateBy = query
	return oq
}
...
func (oq *OrganizationQuery) WithUpdateBy(opts ...func(*UserQuery)) *OrganizationQuery {
	query := &UserQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withUpdateBy = query
	return oq
}
...
func (oq *OrganizationQuery) WithUnits(opts ...func(*OrgUnitQuery)) *OrganizationQuery {
	query := &OrgUnitQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) WithStaffs(opts ...func(*UserQuery)) *OrganizationQuery {
	query := &UserQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(organization.FieldCreateByUser).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Organization.Query().
//		Select(organization.FieldCreateByUser).
//		Scan(ctx, &v)
//
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context) ([]*Organization, error) {
	...
	if query := oq.withCreateBy; query != nil {
	...
	if query := oq.withUpdateBy; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Organization)
		for i := range nodes {
	...
	if query := oq.withUnits; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Organization)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Units = []*OrgUnit{}
		}
		query.withFKs = true
		query.Where(predicate.OrgUnit(func(s *sql.Selector) {
			s.Where(sql.InValues(organization.UnitsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.organization_units
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "organization_units" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "organization_units" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Units = append(node.Edges.Units, n)
		}
	}

	if query := oq.withStaffs; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Organization, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Staffs = []*User{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(organization.StaffsPrimaryKey[0], fks...))
			},
		...
		_spec.Edge.Schema = oq.schemaConfig.OrganizationStaffs
