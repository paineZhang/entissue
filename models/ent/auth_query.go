...
func (aq *AuthQuery) QueryBelongTo() *UserQuery {
...
func (aq *AuthQuery) Clone() *AuthQuery {
	...
	return &AuthQuery{
		...
		withBelongTo: aq.withBelongTo.Clone(),
...
func (aq *AuthQuery) WithBelongTo(opts ...func(*UserQuery)) *AuthQuery {
	query := &UserQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withBelongTo = query
	return aq
}
...
//
//	client.Auth.Query().
//		GroupBy(auth.FieldLastAuthTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (aq *AuthQuery) sqlAll(ctx context.Context) ([]*Auth, error) {
	...
	_spec.Node.Schema = aq.schemaConfig.Auth
	...
	if query := aq.withBelongTo; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_auth_histories" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (aq *AuthQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(aq.schemaConfig.Auth)
	ctx = internal.NewSchemaConfigContext(ctx, aq.schemaConfig)
